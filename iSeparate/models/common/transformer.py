"""
Parts of this code are adapted from https://github.com/facebookresearch/demucs
...
def create_sin_embedding_cape(
    ...
    # We aim for TBC format
    assert dim % 2 == 0
    pos = 1.0 * torch.arange(length, device=device).view(-1, 1, 1)  # (length, 1, 1)
    pos = pos.repeat(1, batch_size, 1)  # (length, batch_size, 1)
    if mean_normalize:
        pos -= torch.nanmean(pos, dim=0, keepdim=True)

    if augment:
        delta = torch.FloatTensor(1, batch_size, 1, device=device).uniform_(-max_global_shift, max_global_shift)
        # delta = np.random.uniform(
        #     -max_global_shift, +max_global_shift, size=[1, batch_size, 1]
        # )
        delta_local = torch.FloatTensor(length, batch_size, 1, device=device).uniform_(-max_local_shift,
                                                                                       max_local_shift)
        # delta_local = np.random.uniform(
        #     -max_local_shift, +max_local_shift, size=[length, batch_size, 1]
        # )
        log_lambdas = torch.FloatTensor(1, batch_size, 1, device=device).uniform_(-np.log(max_scale), np.log(max_scale))
        # log_lambdas = np.random.uniform(
        #     -np.log(max_scale), +np.log(max_scale), size=[1, batch_size, 1]
        # )
        pos = (pos + delta + delta_local) * torch.exp(log_lambdas)
    ...
    def __init__(
        ...
        if sparse:
            self.self_attn = MultiheadAttention(
                ...
                auto_sparsity=sparsity if auto_sparsity else 0, memory_efficient=False
            )
        ...
        if memory_efficient:
            self.self_attn = MultiheadAttention(
                d_model, nhead, dropout=dropout, batch_first=batch_first,
                auto_sparsity=0, memory_efficient=True
            )

    def forward(self, src: Tensor, src_mask: tp.Optional[Tensor] = None,
                src_key_padding_mask: tp.Optional[Tensor] = None) -> Tensor:
        """
...
class CrossTransformerEncoder(nn.Module):
    def __init__(
            ...
            memory_efficient_self_attn: bool = False,
            memory_efficient_cross_attn: bool = False,
            mask_type: str = "diag",
    ...
    def forward(self, x, xt):
        ...
        for idx in range(self.num_layers):
            if idx % 2 == self.classic_parity:
                x = self.layers[idx](x, None, None)
                xt = self.layers_t[idx](xt, None, None)
            else:
                old_x = x
...
class MultiheadAttention(nn.Module):
    ...
    def forward(
        ...
        if self.auto_sparsity:
            ...
            q, k, v = map(lambda t: rearrange(t, '(b h) n d -> b h n d', h=self.num_heads), (q, k, v))
            x = memory_efficient_attention(q, k, v, dropout=self.attn_drop.p,
                                           # q_bucket_size=32, k_bucket_size=32
                                           )
            x = rearrange(x, 'b h n d -> (b h) n d')
        else:
...
# def memory_efficient_attention(query, key, value, dropout=0.1, attn_bias=None):
#     from xformers.ops import memory_efficient_attention
#     y = memory_efficient_attention(query, key, value, p=dropout, attn_bias=attn_bias)
#     return y
